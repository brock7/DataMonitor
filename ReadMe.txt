...
应用程序向导已为您创建了此 DataMonitor 应用程序。此应用程序
不仅介绍了使用 Microsoft 基础类的基本知识，
而且是编写应用程序的起点。

此文件包含组成 DataMonitor 应用程序的每个文件的内容摘要。
...
DataMonitor.h
    ...
    CDataMonitorApp 应用程序类。

DataMonitor.cpp
    这是包含应用程序
    类 CDataMonitorApp 的主应用程序源文件。
...
LeftView.h、LeftView.cpp
    这些文件包含左框架类 CLeftView，此类派生自
    CTreeView。
res\Toolbar.bmp
    此位图文件用于创建工具栏的平铺图像。
    初始工具栏和状态栏在 CMainFrame 
    类中构造。 使用资源编辑器编辑此工具栏位图，并
    更新 DataMonitor.rc 中的 IDR_MAINFRAME TOOLBAR 数组以添加
    工具栏按钮。
/////////////////////////////////////////////////////////////////////////////

应用程序向导将创建一种文档类型和一个视图：

DataMonitorDoc.h、DataMonitorDoc.cpp - 文档
    这些文件包含 CDataMonitorDoc 类。  编辑这些文件
    以添加特殊的文档数据并实现文件的保存和加载
    （通过 CDataMonitorDoc::Serialize）。
DataMonitorView.h、DataMonitorView.cpp - 文档视图
...
StdAfx.h、StdAfx.cpp
    这些文件用于生成名为 DataMonitor.pch 的预编译头文件 (PCH)
